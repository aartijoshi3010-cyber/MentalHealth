...
/* For all pages, use this background image */
...
if choice == "Home":
    ...
    with left:
        ...
        st.markdown("**Features:** mood logging (emoji), journal notes, habit tracker, charts, CSV export, resources & hotlines.")
    with right:
        with st.form(key="signup_form_v1"):
            ...
            if submitted_signup:
                if not su_name.strip() or not su_email.strip() or not su_password:
                    ...
                    if ok:
                        st.success("Account created! Now log in below.")
                    else:
                        st.error(err)
...
# -----------------------------
    ...
    with right:
        ...
        if moods:
            ...
            freq.plot(kind="bar", ax=ax1, color="#3f51b5")
            ...
            ax2.plot(df_moods["Created"], df_moods["MoodNum"], marker="o", linestyle="-", color="#3f51b5")
